...
export async function createCategory({ category, description = "", subcategories = [], iconSrc = "", iconS3Key = "" }) {
  ...
  const document = {
    ...
    iconS3Key: String(iconS3Key || "").trim(),
  ...
  return {
    ...
    slug,
    folder: slug,
    iconSrc: document.iconSrc,
    iconS3Key: document.iconS3Key,
    subcategories: document.subcategories,
    items: [],
  }
}

export async function updateCategory(id, { category, description = "", subcategories = [], iconSrc, iconS3Key }) {
  const name = String(category || "").trim()
  const normalizedDescription = String(description || "").trim()
  if (!name) {
    throw new Error("Category name is required")
  ...
  await db.collection("categories").updateOne(
    { _id: objectId },
    {
      $set: {
        ...
        description: normalizedDescription,
        slug: slugify(name),
        iconSrc: typeof iconSrc === "string" ? iconSrc.trim() : current.iconSrc || "",
        iconS3Key: typeof iconS3Key === "string" ? iconS3Key.trim() : current.iconS3Key || "",
        subcategories: normalizeSubcategories(subcategories),
        updatedAt: new Date(),
      },
    },
